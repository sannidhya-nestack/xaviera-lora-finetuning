...
class Pipeline(LightningModule):
    def __init__(
        ...
        self.is_train = train
        self.T = T
        self.final_checkpoint_dir = checkpoint_dir  # Store for copying final model

        # Choose device & dtype - move models to GPU to save CPU RAM
        # On SageMaker ml.g6.xlarge, this uses 24GB VRAM instead of 16GB RAM
        self.model_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # Use bfloat16 for model weights to halve VRAM usage (~7.7 GB instead of ~15.5 GB)
        # Combined with bf16-mixed precision training, this is memory efficient and stable
        ...
        # step 1: load ACE-Step pipeline & move big models to GPU to save CPU RAM
        acestep_pipeline = ACEStepPipeline(checkpoint_dir)
        acestep_pipeline.load_checkpoint(acestep_pipeline.checkpoint_dir)

        # Main 3.5B transformer - move to GPU instead of CPU
        transformers = acestep_pipeline.ace_step_transformer.to(
            device=self.model_device, dtype=model_dtype
        )
        ...
        if lora_config_path is not None:
            ...
            # Use ACE-Step's built-in adapter API
        ...
        # DCAE and text encoder - keep them frozen, move to GPU to save CPU RAM
        self.dcae = acestep_pipeline.music_dcae.to(device=self.model_device, dtype=model_dtype)
        ...
        # We don't need the full pipeline object anymore - free its memory
        del acestep_pipeline
        gc.collect()
        if self.model_device.type == "cuda":
            torch.cuda.empty_cache()
            # Debug: verify models are on GPU
            print(f"[DEBUG] Model devices after init:")
            print(f"  transformers: {next(self.transformers.parameters()).device}")
            print(f"  dcae: {next(self.dcae.parameters()).device}")
            print(f"  text_encoder: {next(self.text_encoder_model.parameters()).device}")
            print(f"  GPU memory allocated: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")

        if self.is_train:
            self.transformers.train()

            # SSL models - move to GPU to speed up inference (we have ~11 GB headroom)
            # Store in dict to avoid DDP issues, but they'll be on GPU
            self._ssl_models = {}

            try:
                mert_model = AutoModel.from_pretrained(
                    "m-a-p/MERT-v1-330M", trust_remote_code=True, cache_dir=checkpoint_dir
                ).to(device=self.model_device, dtype=model_dtype).eval()
            except:
            ...
            mert_model.requires_grad_(False)
            self._ssl_models['mert'] = mert_model
            self._ssl_models['resampler_mert'] = torchaudio.transforms.Resample(
            ...
            hubert_model = AutoModel.from_pretrained("utter-project/mHuBERT-147").to(
                device=self.model_device, dtype=model_dtype
            ).eval()
            hubert_model.requires_grad_(False)
            self._ssl_models['hubert'] = hubert_model
            self._ssl_models['resampler_mhubert'] = torchaudio.transforms.Resample(
    ...
    def on_train_start(self):
        # Verify models are on GPU after Lightning moves the module
        if torch.cuda.is_available():
            print(f"[DEBUG] Model devices after Lightning init:")
            print(f"  transformers: {next(self.transformers.parameters()).device}")
            print(f"  dcae: {next(self.dcae.parameters()).device}")
            print(f"  text_encoder: {next(self.text_encoder_model.parameters()).device}")
            if hasattr(self, '_ssl_models') and 'mert' in self._ssl_models:
                print(f"  mert: {next(self._ssl_models['mert'].parameters()).device}")
                print(f"  hubert: {next(self._ssl_models['hubert'].parameters()).device}")
            print(f"  GPU memory allocated: {torch.cuda.memory_allocated() / 1024**3:.2f} GB")
            print(f"  GPU memory reserved: {torch.cuda.memory_reserved() / 1024**3:.2f} GB")

    def infer_mert_ssl(self, target_wavs, wav_lengths):
        # MERT runs on GPU for faster inference (we have headroom)
        # Inputs are already on GPU from the batch
        # Input is N x 2 x T (48kHz), convert to N x T (24kHz), mono
        resampler_mert = self._ssl_models['resampler_mert']
        mert_input_wavs_mono_24k = resampler_mert(target_wavs.float().mean(dim=1))
        ...
        with torch.no_grad():
            ...
            mert_model = self._ssl_models['mert']
        ...
        # Organize hidden states by audio (already on GPU)
        ...
        for i in range(bsz):
            ...
            audio_hidden = torch.cat(audio_chunks, dim=0)  # Already on GPU
    ...
    def infer_mhubert_ssl(self, target_wavs, wav_lengths):
        # mHuBERT runs on GPU for faster inference (we have headroom)
        # Inputs are already on GPU from the batch
        # Step 1: Preprocess audio
        # Input: N x 2 x T (48kHz, stereo) -> N x T (16kHz, mono)
        resampler_mhubert = self._ssl_models['resampler_mhubert']
        mhubert_input_wavs_mono_16k = resampler_mhubert(target_wavs.float().mean(dim=1))
        bsz = target_wavs.shape[0]
        ...
        # Step 6: Batch inference with MHubert model on GPU
        with torch.no_grad():
            hubert_model = self._ssl_models['hubert']
            mhubert_ssl_hidden_states = hubert_model(all_chunks).last_hidden_state
        ...
        # Step 9: Reorganize hidden states by original audio (already on GPU)
    ...
    def preprocess(self, batch, train=True):
        ...
        # SSL constraints
        # Lightning's bf16-mixed precision already handles autocast for training steps
    ...
    def run_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.global_step == 0:
            print(f"[DEBUG] Before transformer forward:")
            print(f"  x device: {x.device}, dtype: {x.dtype}")
            print(f"  transformer device: {next(self.transformers.parameters()).device}")
        ...
        if batch_idx == 0 and self.global_step == 0:
            print(f"[DEBUG] After transformer forward:")
    ...
    def on_train_batch_end(self, outputs, batch, batch_idx):
        # Save LoRA adapter periodically (instead of full Lightning checkpoints)
        # This saves only the small LoRA weights, not the entire model state
        if self.global_step % self.hparams.every_plot_step == 0:
            log_dir = self.logger.log_dir
            checkpoint_name = f"step={self.global_step}_lora"
    ...
    def on_train_end(self):
        # Save final LoRA adapter directly to /opt/ml/model for SageMaker upload
        # This is the only file SageMaker will upload (small, no OOM)
        if self.final_checkpoint_dir:
            final_model_dir = os.path.join(self.final_checkpoint_dir, "lora_adapter")
            os.makedirs(final_model_dir, exist_ok=True)
            self.transformers.save_lora_adapter(final_model_dir, adapter_name=self.adapter_name)
            print(f"[INFO] Saved final LoRA adapter to {final_model_dir}")
...
def main(args):
    ...
    # No ModelCheckpoint callback - we save LoRA adapters directly in on_train_batch_end
    # This avoids saving large full model checkpoints that cause OOM during upload
    ...
    trainer = Trainer(
        ...
        callbacks=[],  # No checkpoint callback - we save LoRA adapters directly
...
if __name__ == "__main__":
    ...
    args.add_argument("--s3_output_path", type=str, default=None, help="S3 output path (ignored, SageMaker handles this)")
